...
public class OkHttpUtil {
     ...
     */
    public static void enqueue(String url,Request request, Callback responseCallback) {
        LogInputUtil.e(TAG, "OKHttp enqueue");
        try {
            mOkHttpClient.newCall(request).enqueue(responseCallback);
        } catch (Exception e) {
            LogInputUtil.e(TAG, "异常请求,url ="+url);
        }
    }
     ...
     * @param request
     */
    public static void enqueue(final String url, final Request request) {
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                String failureStr = "访问失败，msg = " + e.getMessage()+",url ="+url;
                MyLog.inputLogToFile(TAG, failureStr, true);
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                String responseStr = "访问成功，body = " + response.body().string()+",url ="+url;
